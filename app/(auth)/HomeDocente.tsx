// 📚 HomeDocente.tsx — sin estilos propios, solo orquestación.
...
import { useHomeDocente } from '../../src/hooks/dashboard/useHomeDocente'
...
export default function HomeDocente() {
  ...
  const { usuario, reportes, stats, cargando, refrescando, error, cargar, onRefresh } = useHomeDocente()
  ...
  if (cargando) return <LoadingScreen color="#21D0B2" />
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F5F7FA' }} edges={['top']}>
      <DashboardLayout nombre={usuario?.nomUser || 'Usuario'}
        refrescando={refrescando} onRefresh={onRefresh}>
        ...
        {reportes.length > 0 && (
          <SectionContainer title="Reportes Recientes" actionLabel="Ver todos →"
            onAction={() => router.push('/ListadoReportes')} padded={false}>
